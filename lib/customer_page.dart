...
class _CustomersPageState extends State<CustomersPage> {
  ...
  Future<void> _deleteCustomer(String userId, String userName) async {
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Row(
          children: [
            ...
            SizedBox(width: 8),
            Text('Delete Customer', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          ],
        ),
        content: Text(
    ...
    if (confirmed == true) {
      try {
        final callable = FirebaseFunctions.instanceFor(region: 'us-central1').httpsCallable('deleteUser');
        await callable.call({'userId': userId});

        setState(() => customers.removeWhere((c) => c['id'] == userId));
  ...
  Future<void> _updateEmail(String userId, String newEmail) async {
    try {
      final callable = FirebaseFunctions.instanceFor(region: 'us-central1').httpsCallable('updateUserEmail');
      await callable.call({'userId': userId, 'newEmail': newEmail});

      setState(() {
        final index = customers.indexWhere((c) => c['id'] == userId);
        if (index != -1) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Email update failed: ${e.message}'), backgroundColor: Colors.red),
      );
    }
  }

  Future<void> _editCustomer(Map<String, dynamic> customer) async {
    final nameController    = TextEditingController(text: customer['full_name'] ?? customer['name'] ?? '');
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            const Text('Edit Customer', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          ],
        ),
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              _editField('Address', addressController, Icons.home),
            ],
          ),
        ),
        actions: [
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            const SizedBox(width: 8),
            const Text('Confirm Changes', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          ],
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          ...
          children: [
            ...
            const SizedBox(height: 8),
            _confirmRow('Address', addressController.text.trim()),
          ],
        ),
        actions: [
          TextButton(
    ...
    if (confirmed == true) {
      ...
      if (newEmail.isNotEmpty && newEmail != oldEmail) {
        await _updateEmail(customer['id'], newEmail);
      }
            ...
            child: Row(
              children: [
                ...
                const Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Customers', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.white)),
                    SizedBox(height: 4),
                    Text('Manage customer accounts', style: TextStyle(fontSize: 14, color: Colors.white70)),
                  ],
                ),
              ],
            ),
                      ...
                      child: Row(
                        children: [
                          ...
                          IconButton(
                            ...
                            icon: Icon(Icons.edit, color: primaryGreen, size: 22),
                          ),
                          IconButton(
                            onPressed: () => _deleteCustomer(customer['id'], name),
